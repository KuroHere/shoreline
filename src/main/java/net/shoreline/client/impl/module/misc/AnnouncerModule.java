...
public class AnnouncerModule extends ToggleModule {
    ...
    //
    Config<Boolean> joinConfig = new BooleanConfig("Join", "Announces when a player joins in the chat", true);
    Config<Boolean> leaveConfig = new BooleanConfig("Leave", "Announces when a player leaves in the chat", true);
    Config<Boolean> walkConfig = new BooleanConfig("Walk", "Announces when you walk in the chat", false);
    Config<Boolean> eatConfig = new BooleanConfig("Eat", "Announces when you eat food in the chat", false);
    Config<Boolean> placeConfig = new BooleanConfig("Place", "Announces when you place blocks in the chat", false);
    Config<Boolean> breakConfig = new BooleanConfig("Break", "Announces when you break blocks in the chat", false);
    Config<Boolean> attackConfig = new BooleanConfig("Attack", "Announces when you attack players in the chat", false);
    Config<Boolean> jumpConfig = new BooleanConfig("Jump", "Announces when you jump in the chat", false);
    Config<Boolean> worldTimeConfig = new BooleanConfig("WorldTime", "Announces the world time in chat", false);
    Config<Boolean> clientSideConfig = new BooleanConfig("ClientSideOnly", "Only announces messages in the clientside chat", false);
    //
    ...
    public void onTick(TickEvent event) {
        ...
        if (worldTimeConfig.getValue()) {
            ...
            if (time == 0) // Start day
            {
                sendAnnouncement(MORNING_ANNOUNCE[RANDOM.nextInt(MORNING_ANNOUNCE.length)]);
            } else if (time == 6000) // Mid day
            {
                sendAnnouncement(AFTERNOON_ANNOUNCE[RANDOM.nextInt(AFTERNOON_ANNOUNCE.length)]);
            } else if (time == 11616) // Sunset
            {
                sendAnnouncement(SUNSET_ANNOUNCE[RANDOM.nextInt(SUNSET_ANNOUNCE.length)]);
            } else if (time == 12000) // Evening
            {
                sendAnnouncement(EVENING_ANNOUNCE[RANDOM.nextInt(EVENING_ANNOUNCE.length)]);
            } else if (time == 18000) // Night
            {
                sendAnnouncement(NIGHT_ANNOUNCE[RANDOM.nextInt(NIGHT_ANNOUNCE.length)]);
            } else if (time == 23448) {
                sendAnnouncement(SUNRISE_ANNOUNCE[RANDOM.nextInt(SUNRISE_ANNOUNCE.length)]);
            }
        }
    }
    ...
    public void onPacketOutbound(PacketEvent.Outbound event) {
        ...
        if (event.getPacket() instanceof PlayerInteractBlockC2SPacket packet
            ...
            blocksPlaced++;
            if (placeAnnounceTimer.passed(150) && blocksPlaced > RANDOM.nextInt(10)) {
    ...
    public void onPacketInbound(PacketEvent.Inbound event) {
        ...
        if (event.getPacket() instanceof PlayerListS2CPacket packet
            ...
            for (PlayerListS2CPacket.Entry entry : packet.getEntries()) {
                ...
                if (player != null) {
                    if (packet.getActions().contains(PlayerListS2CPacket.Action.ADD_PLAYER)) {
                        ...
                        sendAnnouncement(JOIN_ANNOUNCE[RANDOM.nextInt(JOIN_ANNOUNCE.length)]
            ...
            for (UUID id : packet.profileIds()) {
                ...
                if (player != null) {
                    sendAnnouncement(LEAVE_ANNOUNCE[RANDOM.nextInt(LEAVE_ANNOUNCE.length)] + player.getName());
                }
            }
        }
    }
    ...
    public void onBreakBlock(BreakBlockEvent event) {
        ...
        if (breakConfig.getValue() && breakAnnounceTimer.passed(300)
                && blocksBroken > RANDOM.nextInt(10)) {
