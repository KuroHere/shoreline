...
public class MicroshitAuthenticator {
    ...
    private static final String OAUTH_AUTH_URL = "https://login.live.com/oauth20_authorize.srf?client_id=000000004C12AE6F&redirect_uri=https://login.live.com/oauth20_desktop.srf&scope=service::user.auth.xboxlive.com::MBI_SSL&display=touch&response_type=token&locale=en";
     ...
     */
    public Session login(String email, String password) throws MicroshitException, IOException {
        MinecraftProfile profile = getProfileData(email, password);
        if (profile != null) {
            return new Session(profile.username(),
                    profile.id(),
                    profile.accessToken(),
                    Optional.empty(),
                    Optional.empty(),
     ...
     * @throws MicroshitException
     */
    public MinecraftProfile getProfileData(String email, String password) throws IOException, MicroshitException {
        OAuthData oauthData = getOAuthData();
        if (oauthData == null) {
            throw new NullPointerException("OAuthData should not be null");
        }
        ...
        if (xboxLogin == null) {
            throw new MicroshitException("Xbox Login data should not be null");
        }

        XboxLoginData xstsData = getXSTSData(xboxLogin);
        if (xstsData == null) {
            throw new MicroshitException("XSTS data should not be null");
        }
     ...
     */
    public MinecraftProfile getProfile(String accessToken, String refreshToken) throws IOException {
        ...
        JsonObject obj = ConfigFile.PARSER.parse(data).getAsJsonObject();
    ...
    private String authWithXbox(XboxLoginData data) {
        ...
        String content = RequestUtil.post(
                LOGIN_WITH_XBOX_URL,
                headers,
                false,
                "{\"identityToken\":\"XBL3.0 x=" + data.uhs() + ";" + data.token() + "\",\"ensureLegacyEnabled\":true}");
        return content == null ? null : ConfigFile.PARSER.parse(content).getAsJsonObject().get("access_token").getAsString();
    }
    ...
    private XboxLoginData loginToXbox(String accessToken) {
        ...
        String data = RequestUtil.post(XBOX_LIVE_AUTH_URL, headers, false, body);
        if (data == null) {
            return null;
        }
        ...
        String uhs = obj.get("DisplayClaims")
                .getAsJsonObject().get("xui")
                .getAsJsonArray().get(0)
                .getAsJsonObject().get("uhs").getAsString(); // wtf
     ...
     */
    private XboxLoginData getXSTSData(XboxLoginData data) throws IOException, MicroshitException {
        ...
        if (content == null) {
            throw new MicroshitException("input stream cannot be null");
        }

        JsonObject jsonObject = ConfigFile.PARSER.parse(content).getAsJsonObject();

        if (connection.getResponseCode() == 401) {
            long errCode = jsonObject.get("XErr").getAsLong();
            if (errCode == 2148916238L) {
                throw new MicroshitException("account is owned by user under the age of 18");
            } else if (errCode == 2148916233L) {
                throw new MicroshitException("account does not have an xbox account created. please create one");
            }
     ...
     */
    private Map<String, String>  getLoginData(OAuthData oauthData, String email, String password)
        ...
        if (redirectedUrl.contains("accessToken") && redirectedUrl.equals(oauthData.postUrl())) {
            throw new MicroshitException("Login failed, [url=" + redirectedUrl + "]");
        }

        if (data.contains("Sign in to")) {
            throw new MicroshitException("Invalid credentials");
        }
